    ...
    BehaviorSpec({
        ...
        Given("주문 생성 시") {
            ...
            When("여러 상품으로 주문을 생성하면") {
                ...
                val multiItemRequests = listOf(
                    OrderManager.OrderItemRequest(productId = productId, quantity = 2),
                    OrderManager.OrderItemRequest(productId = productId2, quantity = 3),
                )
                val multiProducts = listOf(
                    ProductInfo(
                        id = productId,
                        storeId = storeId,
                        storeName = "Test Store",
        ...
        Given("주문 취소 시") {
            ...
            When("취소 가능한 상태의 본인 주문을 취소하면") {
                ...
                val cancelEvent = orderManager.cancelOrder(
                    order = order,
                    userId = userId,
                    cancelReason = "고객 변심",
                    now = now,
